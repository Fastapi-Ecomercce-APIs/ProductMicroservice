...
@router.post("/create", response_model=schemas.ProductoSend, status_code=status.HTTP_201_CREATED, summary="Crear un producto", description="Crea un nuevo producto , el campo imagen es opciona, asefurate que sea una url")
...
@router.get("/get_all", response_model=List[schemas.ProductoSend], status_code=status.HTTP_200_OK)
async def get_all_products(service:models_repository.ProductService=Depends(get_product_service)):
...
async def delete_product(product_id,service:models_repository.ProductService=Depends(get_product_service)):
...
async def get_by_category(category_id, service:models_repository.ProductService=Depends(get_product_service)):
